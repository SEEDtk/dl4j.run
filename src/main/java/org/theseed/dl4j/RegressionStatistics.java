...
public class RegressionStatistics {
    ...
    public void finish() {
        ...
        int floor = (this.errors.length - 1) >> 1;
        int ceil = this.errors.length >> 1;
        ...
        floor = (this.errors.length - 2) >> 2;
        ceil = this.errors.length >> 2;
        ...
        int length3 = this.errors.length * 3;
        floor = (length3 - 1) >> 2;
        ceil = (length3 + 1) >> 2;
    ...
    public double trimmedMean(double reduce) {
        ...
        int lowIdx = (int) Math.floor(reduce * this.errors.length);
        int highIdx = this.errors.length - 1 - lowIdx;
